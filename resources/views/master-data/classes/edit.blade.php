    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h3 class="font-bold text-lg text-gray-800">Form Edit Kelas</h3>
                <p class="text-sm text-gray-500 mt-1">Ubah data kelas di bawah ini</p>
            </div>

            <form method="POST" action="{{ route('master-data.classes.update', $class) }}" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="level" id="level" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm">
                        @for($i = 1; $i <= 6; $i++)
                            <option value="{{ $i }}" {{ old('level', $class->level) == $i ? 'selected' : '' }}>Kelas {{ $i }}</option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="academic_year" id="academic_year" value="{{ old('academic_year', $class->academic_year) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm">
